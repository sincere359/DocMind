...
| 🕸️ **知识图谱** | LLM 提取实体关系，可视化图谱 | mimo-v2.5 |
| ⚡ **流式输出** | 实时 token 流式响应 | mimo-v2.5-pro |
| 🔐 **用户认证** | 注册/登录/JWT，per-user 索引隔离 | — |
| 🐳 **Docker 部署** | 一键容器化部署，模型预下载 | — |
...
| 文档解析 | pdfplumber + python-docx + openpyxl + xlrd + python-pptx | 7 种格式 |
| 知识图谱 | streamlit-agraph | 交互式网络图可视化 |
| REST API | FastAPI + uvicorn | 生产级 API 服务 |
| 认证 | SQLite + JWT | per-user 索引隔离 |
...
# 或 FastAPI 后端
python -m docmind.api --port 8000 --reload
```

浏览器打开 `http://localhost:8501` (Streamlit) 或 `http://localhost:8000/docs` (API 文档)。
...
1. **注册/登录**：首次使用需注册账号，登录后进入主界面
2. **上传文档**：侧边栏上传 PDF/TXT/MD/DOCX/XLSX/XLS/PPTX 文件
3. **索引文档**：点击"索引文档"按钮，自动解析分块并建立向量索引
4. **智能问答**：基于文档内容提问，AI 精准回答并标注来源
5. **文档摘要**：一键生成详细/简短/学术风格摘要
6. **信息提取**：自动提取实体、数据、结论、风险、建议
7. **文档对比**：选择两个文档，AI 分析异同
8. **知识图谱**：LLM 提取实体与关系，交互式可视化展示
...
│   ├── document_parser.py   # 文档解析 + 分块 (7 格式)
│   ├── embeddings.py        # 文本向量化
